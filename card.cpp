					     ...
					     SDL_Renderer *renderer)
{
	...
	struct flipclock_card *card = (struct flipclock_card *)malloc(sizeof(*card));
	if (card == NULL) {
		blog(LOG_ERROR, "[flipclock] Failed to create card!");
		return NULL;
	}
...
static void _flipclock_card_create_textures(struct flipclock_card *card)
{
	...
	blog(LOG_INFO, "[flipclock] Creating new textures with size `%dx%d`.\n", card->rect.w,
		  card->rect.h);
	card->current = SDL_CreateTexture(card->renderer, 0,
					  SDL_TEXTUREACCESS_TARGET,
					  card->rect.w, card->rect.h);
	if (card->current == NULL) {
		blog(LOG_ERROR, "[flipclock] failed to create current texture: %s\n", SDL_GetError());
		return;
	}
	...
	if (card->previous == NULL) {
		blog(LOG_ERROR, "[flipclock] failed to create previous texture: %s\n", SDL_GetError());
		return;
	}
...
static void _flipclock_card_destroy_textures(struct flipclock_card *card)
{
	...
	blog(LOG_INFO, "[flipclock] Destroying textures.\n");
	if (card->current != NULL) {
...
static void _flipclock_card_open_fonts(struct flipclock_card *card)
{
	...
	blog(LOG_INFO, "[flipclock] Opening font from `%s`.\n", app->font_path);
	card->font =
	...
	if (card->font == NULL || card->sub_font == NULL) {
		blog(LOG_ERROR, "[flipclock] failed to open font: %s\n", TTF_GetError());
		return;
	}
}
...
static void _flipclock_card_close_fonts(struct flipclock_card *card)
{
	...
	blog(LOG_INFO, "[flipclock] Closing old font.\n");
	if (card->font != NULL) {
		       ...
		       const char text[])
{
	...
	for (int i = 0; i < len; ++i) {
		...
		if (text_surface == NULL) {
			blog(LOG_ERROR, "[flipclock] failed to blend: %s\n", TTF_GetError());
			break;
		}
		SDL_Texture *text_texture =
			SDL_CreateTextureFromSurface(renderer, text_surface);
		if (text_texture == NULL) {
			blog(LOG_ERROR, "[flipclock] failed to create texture from surface: %s\n", TTF_GetError());
			break;
		}
		SDL_Rect text_rect;
